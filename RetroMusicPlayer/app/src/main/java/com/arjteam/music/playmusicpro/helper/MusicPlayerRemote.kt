...
object MusicPlayerRemote : KoinComponent {
    ...
    fun playNext(song: Song): Boolean {
        if (musicService != null) {
            ...
            Toast.makeText(
                musicService,
                musicService!!.resources.getString(com.arjteam.music.playmusicpro.R.string.added_title_to_playing_queue),
                Toast.LENGTH_SHORT
            ).show()
            return true
        }
    ...
    fun playNext(songs: List<Song>): Boolean {
        if (musicService != null) {
            ...
            val toast =
                if (songs.size == 1) musicService!!.resources.getString(com.arjteam.music.playmusicpro.R.string.added_title_to_playing_queue) else musicService!!.resources.getString(
                    com.arjteam.music.playmusicpro.R.string.added_x_titles_to_playing_queue,
                    songs.size
                )
